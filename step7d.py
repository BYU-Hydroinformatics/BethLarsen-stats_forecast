# step6_ytd_forecast_LOYO_scaledmedian.py
# LOYO CV: For each year, train on all other years and forecast the 3-month cumulative increments
# using the scaled median method. Save one plot per validation year and a CSV of metrics.
...
metrics_csv_path = os.path.join(out_folder, "validation_metrics.csv")
...
# MAIN LOYO LOOP
# ==========================================================
metrics = []  # will hold [Year, RMSE_ScaledMedian, R2_ScaledMedian, FinalVolumeDiff]
skipped_years = []
...
for vy in years_all:
    ...
    # metrics: compare true_inc (length n) to scaled_fore_inc (length n)
    # ensure arrays are numeric and aligned
    true_inc_arr = np.asarray(true_inc)
    if len(scaled_fore_inc) != n:
        # safety check (shouldn't happen due to extend_or_trim)
        scaled_fore_inc = extend_or_trim(scaled_fore_inc, n)

    # RMSE
    rmse_scaled = np.sqrt(mean_squared_error(true_inc_arr, scaled_fore_inc))
    # R2 (guard against constant or degenerate cases)
    try:
        r2_scaled = r2_score(true_inc_arr, scaled_fore_inc)
    except Exception:
        r2_scaled = np.nan

    # Final volume difference (observed final cum - forecast final cum)
    final_obs = cum_at_ref + true_inc_arr[-1]
    final_fore = cum_at_ref + scaled_fore_inc[-1]
    final_diff = final_obs - final_fore

    metrics.append([vy, rmse_scaled, r2_scaled, final_diff])
    print(f"Year {vy} | RMSE_scaled={rmse_scaled:.3f} | R2_scaled={np.nan if pd.isna(r2_scaled) else r2_scaled:.3f} | FinalDiff={final_diff:.3f}")
    ...
    # Build series for plot: forecasts are cumulative starting from cum_at_ref
    # scaled_fore cumulative = cum_at_ref + scaled_fore_inc
    ...
    if len(obs_post_index) != n:
        # create artificial index by using ref_date + days (1..n)
        obs_post_index = pd.date_range(start=ref_date, periods=n, freq='D')
        # if original post window had less frequent sampling, this synthetic index is acceptable for plotting
    ...
    plt.plot(obs_post_index, med_cum, color="green", label="Median forecast")
    plt.plot(obs_post_index, scaled_cum, color="purple", label="Scaled median forecast")
    ...
    plt.title(f"{vy} — 9-Month History + 3-Month Forecast (Scaled Median)")
    plt.xlabel("Date")
...
# SAVE METRICS CSV
# ==========================================================
metrics_df = pd.DataFrame(metrics, columns=["Year", "RMSE_ScaledMedian", "R2_ScaledMedian", "FinalVolumeDiff"])
