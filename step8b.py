...
out_folder = "/Users/bethlarsen/Downloads/Hydro Lab/stat_forecast_project/forecast_plots_pwr"
os.makedirs(out_folder, exist_ok=True)
metrics_csv_path = os.path.join(out_folder, "validation_metrics_pwr.csv")

# ==========================================================
# USER-TUNABLE
# ==========================================================
nonlinear_min_points = 4  # minimum pre-window points to attempt nonlinear fit
...
# MAIN LOYO LOOP
# ==========================================================
metrics = []  # will hold [Year, RMSE_Nonlinear, R2_Nonlinear, FinalVolumeDiff_Nonlinear]
skipped_years = []

def power_model(x, a, b):
    """Nonlinear power model used for fitting (handles negative x)."""
...
for vy in years_all:
    ...
    # Forecast arrays are constructed with a leading 0 at day 0 (ref day), then day1..day_{n-1}
    ...
    # -------------------------
    # Build median cumulative curve for post-ref (needed for nonlinear forecast)
    # median_curve_full is cumulative starting at 0 at ref (length n)
    median_curve_full = np.concatenate(([0], med_for_days)).astype(float)

    # -------------------------
    # PRE-WINDOW data to fit nonlinear transform:
    # For fitting we want pre_obs_series (cumulative) and pre_med_series (median cumulative)
    # Build pre_obs (validation year's pre-window cumulative ending at ref)
    ...
    # Default forecast is scaled median; attempt nonlinear fit and if it fails use fallback
    use_nonlinear = True

    if len(pre_median_list) == 0:
        # cannot build median pre-window -> fallback to scaled median
        print(f"Year {vy}: no training pre-windows available for nonlinear fit. Using scaled median fallback.")
        use_nonlinear = False
    ...
    if use_nonlinear:
        ...
        # If min_len is 0 (shouldn't happen), fallback
        if min_len < 1:
            print(f"Year {vy}: pre-window lengths too short for nonlinear fit. Using scaled median fallback.")
            use_nonlinear = False
        else:
            # Build median pre-window (take last min_len values from each training-pre)
            trimmed_pre_meds = np.array([arr[-min_len:] for arr in pre_median_list])
            pre_med_series = np.median(trimmed_pre_meds, axis=0)

            # Align validation pre_obs to last min_len values
            if len(pre_obs_all) < min_len:
                # Not enough validation pre-window points, fallback
                print(f"Year {vy}: validation pre-window shorter than training pre-window. Using scaled median fallback.")
                use_nonlinear = False
            else:
                pre_obs_series = pre_obs_all[-min_len:]

    if use_nonlinear:
        # compute deviations from start of pre-window
        ...
        if np.sum(mask) < nonlinear_min_points:
            print(f"Year {vy}: insufficient pre-window points ({np.sum(mask)}) for nonlinear model. Using scaled median fallback.")
            use_nonlinear = False

    if use_nonlinear:
        # attempt fit
        try:
            popt, _ = curve_fit(power_model, pre_med_dev[mask], pre_obs_dev[mask], p0=[1.0, 1.0], maxfev=20000)
            a_fit, b_fit = popt
            # Build forecast_dev = b * sign(med_dev) * |med_dev|^a
            med_dev = median_curve_full - median_curve_full[0]
            forecast_dev = b_fit * np.sign(med_dev) * (np.abs(med_dev) ** a_fit)
            nonlinear_inc = forecast_dev.copy()
            nonlinear_inc = extend_or_trim(nonlinear_inc, n)
            nonlinear_inc[0] = 0.0
            print(f"Year {vy}: nonlinear fit succeeded (a={a_fit:.3f}, b={b_fit:.3f})")
        except Exception as e:
            print(f"Year {vy}: nonlinear fit failed ({e}). Using scaled median fallback.")
            use_nonlinear = False

    if not use_nonlinear:
        # Fallback to scaled median forecast
        nonlinear_inc = scaled_fore_inc.copy()
        print(f"Year {vy}: using scaled median forecast as fallback.")

    # Ensure shapes
    nonlinear_inc = extend_or_trim(nonlinear_inc, n)
    true_inc_arr = np.asarray(true_inc)
    if len(nonlinear_inc) != n:
        nonlinear_inc = extend_or_trim(nonlinear_inc, n)

    # RMSE and R2 for nonlinear forecast
    rmse_nl = np.sqrt(mean_squared_error(true_inc_arr, nonlinear_inc))
    try:
        r2_nl = r2_score(true_inc_arr, nonlinear_inc)
    except Exception:
        r2_nl = np.nan

    final_obs = cum_at_ref + true_inc_arr[-1]
    final_fore_nl = cum_at_ref + nonlinear_inc[-1]
    final_diff_nl = final_obs - final_fore_nl

    metrics.append([vy, rmse_nl, r2_nl, final_diff_nl])
    print(f"Year {vy} | RMSE_Nonlinear={rmse_nl:.3f} | R2_Nonlinear={np.nan if pd.isna(r2_nl) else r2_nl:.3f} | FinalDiff_Nonlinear={final_diff_nl:.3f}")
    ...
    nl_cum = cum_at_ref + nonlinear_inc
    ...
    # Plot nonlinear forecast (primary)
    plt.plot(obs_post_index, nl_cum, color="purple", label="Nonlinear forecast")
    ...
    plt.title(f"{vy} — 9-Month History + 3-Month Forecast (Nonlinear Power Forecast)")
    plt.xlabel("Date")
...
# SAVE METRICS CSV
# ==========================================================
metrics_df = pd.DataFrame(metrics, columns=["Year", "RMSE_Nonlinear", "R2_Nonlinear", "FinalVolumeDiff_Nonlinear"])
