...
for vy in years_all:
    ...
    # -----------------------------------------
    # NEW METRICS FOR NONLINEAR POWER METHOD
    # -----------------------------------------
    ...
    # RMSE
    rmse_nl = np.sqrt(np.mean((y - yhat) ** 2))

    # --- NSE ---
    SSE = np.sum((y - yhat) ** 2)
    SST = np.sum((y - y_mean) ** 2)
    nse_nl = 1 - SSE / SST if SST != 0 else np.nan
    ...
    d1_nl = 1 - numerator / denominator if denominator != 0 else np.nan

    # --- R² (traditional) ---
    r2_nl = 1 - SSE / SST if SST != 0 else np.nan

    final_obs = cum_at_ref + true_inc_arr[-1]
    final_fore_nl = cum_at_ref + nonlinear_inc[-1]
    final_diff_nl = final_obs - final_fore_nl

    metrics.append([vy, rmse_nl, nse_nl, d1_nl, r2_nl, final_diff_nl])
