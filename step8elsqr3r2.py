...
# Output folder for plots & csv -> user selected name
out_folder = "/Users/bethlarsen/Downloads/Hydro Lab/stat_forecast_project/forecast_lsqr_results"
...
def compute_r2_corr(obs, pred):
    """Regression-based R^2: squared Pearson correlation coefficient."""
    obs = np.asarray(obs)
    pred = np.asarray(pred)
    if obs.size == 0 or pred.size == 0 or obs.shape != pred.shape:
        return np.nan
...
def compute_nse(obs, pred):
    ...
    if denom == 0:
        return np.nan
    return 1.0 - np.sum((x - y) ** 2) / denom

def compute_willmott_d(obs, pred):
    ...
    x = obs[mask]
    y = pred[mask]
    denom = np.sum((np.abs(y - x.mean()) + np.abs(x - x.mean())) ** 2)
    if denom == 0:
        return np.nan
    return 1.0 - np.sum((x - y) ** 2) / denom
...
# ==========================================================
# MAIN LOYO LOOP (LSQR method); collect metrics
# ==========================================================
metrics = []  # will hold [Year, RMSE_LSQR, R2_corr, NSE, Willmott_d, FinalVolumeDiff_LSQR]
skipped_years = []
...
for vy in years_all:
    ...
    if use_lsqr:
        ...
        if denom == 0.0:
            ...
            k = numer / denom
            # optional: limit k to non-negative (uncomment to enforce)
            # k = max(k, 0.0)
    ...
    # RMSE and the three statistics: R2_corr (corr^2), NSE, Willmott d
    rmse_lsqr = float(np.sqrt(mean_squared_error(true_inc_arr, lsqr_inc)))
    r2_corr = compute_r2_corr(true_inc_arr, lsqr_inc)
    nse_val = compute_nse(true_inc_arr, lsqr_inc)
    willmott_d = compute_willmott_d(true_inc_arr, lsqr_inc)

    final_obs = cum_at_ref + true_inc_arr[-1]
    final_fore_lsqr = cum_at_ref + lsqr_inc[-1]
    final_diff_lsqr = final_obs - final_fore_lsqr

    metrics.append([vy, rmse_lsqr, r2_corr, nse_val, willmott_d, final_diff_lsqr])
    print(f"Year {vy} | RMSE_LSQR={rmse_lsqr:.3f} | R2_corr={np.nan if pd.isna(r2_corr) else r2_corr:.3f} | NSE={np.nan if pd.isna(nse_val) else nse_val:.3f} | d={np.nan if pd.isna(willmott_d) else willmott_d:.3f} | FinalDiff_LSQR={final_diff_lsqr:.3f}")
...
# SAVE METRICS CSV
# ==========================================================
metrics_df = pd.DataFrame(metrics, columns=["Year", "RMSE_LSQR", "R2_corr", "NSE", "Willmott_d", "FinalVolumeDiff_LSQR"])
metrics_df = metrics_df.sort_values("Year").reset_index(drop=True)
metrics_df.to_csv(metrics_csv_path, index=False)
print(f"\nSaved metrics CSV: {metrics_csv_path}")

# Print summary averages (skip NaNs)
if len(metrics_df) > 0:
    avg_rmse = metrics_df["RMSE_LSQR"].mean()
    avg_r2 = metrics_df["R2_corr"].dropna().mean() if "R2_corr" in metrics_df.columns else np.nan
    avg_nse = metrics_df["NSE"].dropna().mean() if "NSE" in metrics_df.columns else np.nan
    avg_d = metrics_df["Willmott_d"].dropna().mean() if "Willmott_d" in metrics_df.columns else np.nan
    print("\nAverage metrics across validation years:")
    print(f"  Mean RMSE_LSQR = {avg_rmse:.3f}")
    print(f"  Mean R2_corr   = {np.nan if pd.isna(avg_r2) else avg_r2:.3f}")
    print(f"  Mean NSE       = {np.nan if pd.isna(avg_nse) else avg_nse:.3f}")
    print(f"  Mean d         = {np.nan if pd.isna(avg_d) else avg_d:.3f}")
